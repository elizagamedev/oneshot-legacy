...
LPWSTR
util_getUnicodeCP(LPCSTR sz8, int cp)
{
    if(!sz8)
        return NULL;
...
void*
util_findMemory(void* start, void* data, size_t size, size_t after)
{
...
LPWSTR
util_getFilenameFromUnicode(LPCWSTR szFile)
{
    //Stupid LSD files
    if(*szFile == '\\')
        szFile++;
...
int
util_messagebox(LPCWSTR text, LPCWSTR title, int type)
{
    if(winver == WIN_WINE)
    {
...
BOOL
util_getResource(const char* name, void** data, size_t* size)
{
    HGLOBAL hRes = NULL;
    HRSRC res;
