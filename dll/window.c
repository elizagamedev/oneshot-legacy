...
//kitty thread
DWORD WINAPI
wndKitty_thread(LPVOID lpParameter)
{
...
LRESULT CALLBACK
wndKitty_proc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch(msg)
	{
	case WM_CLOSE:
...
wndKitty_create()
{
	...
	if(!(wndKitty = CreateWindowEx(
		WS_EX_COMPOSITED | WS_EX_LAYERED | WS_EX_TOPMOST | WS_EX_TOOLWINDOW,
		wndKitty_classname,
		"Kitty",
		WS_POPUP,
		0, 0,
